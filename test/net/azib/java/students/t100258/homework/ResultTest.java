...
public class ResultTest {
	...
	public void testResult() {
		...
		assertEquals("Sprint 100 m", 14.15f, result.sprint_100);
		assertEquals("Long Jump", 3.98f, result.long_jump);
		assertEquals("Short Put", 10.0f, result.short_put);
		assertEquals("High Jump", 1.45f, result.high_jump);
		assertEquals("Sprint 400 m", "70.12", result.sprint_400);
		assertEquals("Hurdles 110 m", 18.65f, result.hurdles);
		assertEquals("Discus Throw", 29.15f, result.discus_throw);
		assertEquals("Pole Vault", 2.15f, result.pole_vault);
		assertEquals("Jevelin Throw", 32.67f, result.jevelin_throw);
