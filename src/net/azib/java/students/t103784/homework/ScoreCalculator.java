...
public class ScoreCalculator {
	...
	public static void calculate(Athlete athlete) {
		...
		for (int i = 0; i < 10; i++) {
			if (i == 0 || i == 4 || i == 5 || i == 9) {
				tempScore += calculateTrackEvents(parameters[i][j], parameters[i][j + 1], parameters[i][j + 2], athlete.getPerformance(i));
			} else if (i == 2 || i == 6 || i == 8) {
				tempScore += calculateThrowEvents(parameters[i][j], parameters[i][j + 1], parameters[i][j + 2], athlete.getPerformance(i));
			} else {
				tempScore += calculateJumpEvents(parameters[i][j], parameters[i][j + 1], parameters[i][j + 2], athlete.getPerformance(i));
			}
			j = 0;
		}
	...
	private static int calculateTrackEvents(double A, double B, double C, double P) {
		return (int) (A * (Math.pow((B - P), C)));
	}

	private static int calculateJumpEvents(double A, double B, double C, double P) {
		return (int) (A * (Math.pow((P * 100 - B), C)));
	}

	private static int calculateThrowEvents(double A, double B, double C, double P) {
