...
public class ConsoleReader implements Reader {
	...
	/**
	 * Empty input strings should not be input
	 *
	 * @return nonempty string value input by used
	 * @throws IOException
	 */
	...
	public String readRecordManualy() throws IOException {
		...
		do {
			tempName = returnIfNotEmpty();
		} while (ValidationOfUserInputData.validateName(tempName) == false);

		recordRawData = recordRawData + DOUBLE_QUOTE + tempName + DOUBLE_QUOTE + COMA_FIELD;
		System.out.println("Please enter participant BirthDate [dd.MM.yyyy]: ");

		do {
			tempBirthDate = returnIfNotEmpty();
		} while (ValidationOfUserInputData.validateBirthDate(tempBirthDate) == false);

		recordRawData = recordRawData + tempBirthDate + COMA_FIELD;
