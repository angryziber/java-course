...
/**This object keeps athlon's results for every of 10 event along with his name, country and birthdate;  Dec-Athlon = ten results of athlon (athlete). */
class Decathlon {
	...
	public int getScore() { // a computable field
		...
		for (int i = 0 ; i != 10 ; i++) {
			...
			total += DecathlonCalculator.resultFactors[i].computePoints(ps[i]);
		}
		return total;
	}
	...
	Decathlon(String csv) throws java.text.ParseException {
		...
		for (int i = 0 ; i != 10 ; i++) // parse all the results translating into P
			ps[i] = DecathlonCalculator.resultFactors[i].formatter.parse(tokenizer.next());
		DecathlonCalculator.list.add(this);
	}

	String toCsv() {
		StringBuffer csv = new StringBuffer(1000);
		csv.append(place + "(" + getScore() + ")");
		csv.append(" - " + DecathlonCalculator.Q + name + DecathlonCalculator.Q + "," + DecathlonCalculator.dateFormat.format(birthdy) + "," + country);
		for (int i = 0 ; i != ps.length ; i++)
			csv.append(",").append(DecathlonCalculator.resultFactors[i].formatter.format(ps[i]));
		return csv.toString();
	}
...
/** A server for managing competitions. */
				...
				new Method("console", false) {
						...
						public void invoke(String argument) throws Exception {
							...
							while (true) {
								...
								// submit csv string
								try {
									new Decathlon(csv.toString()); // instantiate the 
...
//		
							new Using<BufferedInputStream>(new BufferedInputStream(new FileInputStream(argument))) {public void work() throws Exception {
								...
								new Using<BufferedReader>(new BufferedReader(new InputStreamReader(devfile, "UTF-8"))) {public void work() throws Exception {
									// C#: using(Buffer devfile = new BufferedReader(new InputStreamReader(devfile, "UTF-8"))) {
										String csv;
