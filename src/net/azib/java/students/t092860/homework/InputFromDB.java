...
/**
 * Class to be used to read data from database.
 */
public class InputFromDB implements Input {
	 ...
	 * Constructor
	 * 
	 * @param competitionId the id of competition to retrieve data from
	 */
	...
	/**
	 * Gets data of certain competition from the database
	 * 
	 * @return list of athletes data
	 * @throws Exception if database cannot be connected or connection is lost.
	 * 		   Also if incorrect data is received.
	 */
	public List<Data> get() throws Exception {
		List<Data> dataSet = new ArrayList<Data>();

		try {
			String connStr = "jdbc:" + getPropeties().getProperty("db.protocol", "mysql") + "://"
					+ getPropeties().getProperty("db.server", "java.azib.net") + ":"
					+ getPropeties().getProperty("db.port", "3306") + "/"
					+ getPropeties().getProperty("db.database", "decathlon") + "?zeroDateTimeBehavior=convertToNull";
			...
			if (connection != null) {
				try {
					...
					s.executeQuery("SELECT * FROM results " + "LEFT JOIN athletes ON results.athlete_id=athletes.id "
							+ "LEFT JOIN competitions ON results.competition_id=competitions.id  " + "WHERE competitions.id="
							+ competition + " " + "OR competitions.name='" + competition + "'");
